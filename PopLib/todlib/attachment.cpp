...
void AttachmentUpdateAndSetMatrix(AttachmentID& theAttachmentID, Transform2D& theMatrix)
{
	...
	TOD_ASSERT(gEffectSystem);
	Attachment* aAttachment = gEffectSystem->mAttachmentHolder->mAttachments.DataArrayTryToGet((uint32_t)theAttachmentID);
	if (aAttachment)
	{
		aAttachment->Update();
...
//0x405540
AttachEffect* CreateEffectAttachment(AttachmentID& theAttachmentID, EffectType theEffectType, uint32_t theDataID, float theOffsetX, float theOffsetY)
{
	TOD_ASSERT(gEffectSystem);
	Attachment* aAttachment = gEffectSystem->mAttachmentHolder->mAttachments.DataArrayTryToGet((uint32_t)theAttachmentID);
...
{
	uint32_t aReanimId = gEffectSystem->mReanimationHolder->mReanimations.DataArrayGetID(theReanimation);
...
AttachEffect* AttachParticle(AttachmentID& theAttachmentID, TodParticleSystem* theParticleSystem, float theOffsetX, float theOffsetY)
{
	...
	uint32_t aParticleId = gEffectSystem->mParticleHolder->mParticleSystems.DataArrayGetID(theParticleSystem);
...
{
	uint32_t aTrailId = gEffectSystem->mTrailHolder->mTrails.DataArrayGetID(theTrail);
