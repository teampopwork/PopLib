...
#include "zlib.h" // @ThePixelMoon: lol
...
bool DefReadFromCacheArray(void*& theReadPtr, DefinitionArrayDef* theArray, DefMap* theDefMap)
{
    ...
    for (int i = 0; i < theArray->mArrayCount; i++)
        if (!DefMapReadFromCache(theReadPtr, theDefMap, (void*)((uintptr_t)pData + theDefMap->mDefSize * i)))  // 最后一个参数表示 pData[i]
            return false;
    return true;
}
...
bool DefReadFromCacheImage(void*& theReadPtr, Image** theImage)
{
    ...
    char* aImageName = (char*)STACK_ALLOC(aLen + 1);  // 在栈上分配贴图标签字符数组的内存空间
...
bool DefReadFromCacheFont(void*& theReadPtr, Font** theFont)
{
    ...
    char* aFontName = (char*)STACK_ALLOC(aLen + 1);  // 在栈上分配字体标签字符数组的内存空间
...
void* DefinitionUncompressCompiledBuffer(void* theCompressedBuffer, size_t theCompressedBufferSize, size_t& theUncompressedSize, const PopString& theCompiledFilePath)
{
    ...
    Bytef* aSrc = (Bytef*)((uintptr_t)theCompressedBuffer + sizeof(CompressedDefinitionHeader));  // 实际解压数据从第 3 个四字节开始
...
bool DefinitionIsCompiled(const PopString& theXMLFilePath)
{
    ...
    std::filesystem::path compiledPath(aCompiledFilePath.c_str());
    std::filesystem::path xmlPath(theXMLFilePath.c_str());
    ...
    auto xmlTime = std::filesystem::last_write_time(xmlPath);

    return compiledTime > xmlTime;
}
...
void DefinitionFillWithDefaults(DefMap* theDefMap, void* theDefinition)
{
    ...
    for (DefField* aField = theDefMap->mMapFields; *aField->mFieldName != '\0'; aField++)  // 遍历 theDefinition 的每一个成员变量
        if (aField->mFieldType == DefFieldType::DT_STRING)
            *(char**)((uintptr_t)theDefinition + aField->mFieldOffset) = "";  // 将所有 char* 类型的成员变量赋值为空字符数组的指针
}
...
void DefinitionXmlError(XMLParser* theXmlParser, const char* theFormat, ...)
{
    ...
    std::string aFormattedMessage = PopStringToString(vformat(theFormat, argList));
    va_end(argList);
...
bool DefinitionReadIntField(XMLParser* theXmlParser, int* theValue)
{
    ...
    if (sscanf(aStringValue.c_str(), _S("%d"), theValue) == 1)
        return true;
...
bool DefinitionReadFloatField(XMLParser* theXmlParser, float* theValue)
{
    ...
    if (sscanf(aStringValue.c_str(), _S("%f"), theValue) == 1)
        return true;
...
bool DefinitionReadVector2Field(XMLParser* theXmlParser, Vector2* theValue)
{
    ...
    if (sscanf(aStringValue.c_str(), _S("%f %f"), theValue) == 1)
        return true;
...
bool DefinitionReadFlagField(XMLParser* theXmlParser, const PopString& theElementName, uint* theResultValue, DefSymbol* theSymbolMap)
{
    ...
    int aFlag;
    if (sscanf(aStringValue.c_str(), _S("%f %f"), &aFlag) != 1)
    {
        DefinitionXmlError(theXmlParser, "Can't parse int value '%s'", aStringValue.c_str());
        return false;
    }
...
bool DefinitionReadImageField(XMLParser* theXmlParser, Image** theImage)
{
    ...
    std::string aMessgae = StrFormat("Failed to find image '%s' in %s", PopStringToStringFast(aStringValue).c_str(), theXmlParser->GetFileName().c_str());
...
bool DefinitionReadFontField(XMLParser* theXmlParser, Font** theFont)
{
    ...
    std::string aMessgae = StrFormat("Failed to find font '%s' in %s", PopStringToStringFast(aStringValue).c_str(), theXmlParser->GetFileName().c_str());
...
{
    for (int i = 0; i < theArray->mArrayCount; i++)
        DefinitionFreeMap(theDefMap, (void*)((uintptr_t)theArray->mArrayData + theDefMap->mDefSize * i));  // 最后一个参数表示 pData[i]
