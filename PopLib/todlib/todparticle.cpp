...
bool TodParticleLoadADef(TodParticleDefinition* theParticleDef, const char* theParticleFileName)
{
	...
	else
	{
		for (int i = 0; i < theParticleDef->mEmitterDefCount; i++)
		{
			...
			if (aDef.mImage)
				((MemoryImage*)aDef.mImage)->mImageFlags |= TextureFlags::Flag_MinimizeNumSubdivisions;
		}
		return true;
	}
}
...
void TodParticleSystem::ParticleSystemDie()
{
	...
	{
		TodParticleEmitter* aEmitter = mParticleHolder->mEmitters.DataArrayGet((uint32_t)aNode->mValue);
		aEmitter->DeleteAll();
...
{
	TodParticleEmitter* aCrossFadeEmitter = mParticleSystem->mParticleHolder->mEmitters.DataArrayTryToGet((uint32_t)mCrossFadeEmitterID);
...
void TodParticleEmitter::DeleteNonCrossFading()
{
	...
	{
		TodParticle* aParticle = mParticleSystem->mParticleHolder->mParticles.DataArrayGet((uint32_t)aNode->mValue);
...
{
	TodParticle* aCrossFadeParticle = mParticleSystem->mParticleHolder->mParticles.DataArrayTryToGet((uint32_t)theParticle->mCrossFadeParticleID);
...
void TodParticleEmitter::Update()
{
	...
	{
		if (TestBit(mEmitterDef->mParticleFlags, (uint32_t)ParticleFlags::PARTICLE_SYSTEM_LOOPS))  // 判断发射器是否循环
...
bool TodParticleEmitter::GetRenderParams(TodParticle* theParticle, ParticleRenderParams* theParams)
{
	...
	TodParticle* aCrossFadeParticle = aEmitter->mParticleSystem->mParticleHolder->mParticles.DataArrayTryToGet((uint32_t)theParticle->mCrossFadeParticleID);
	...
	if (GetRenderParams(theParticle, &aParams))
	{
		...
		if (aColor.mAlpha > 0)  // 不透明度为 0 时，不绘制
		{
			...
			if (mImageOverride || mEmitterDef->mImage)  // 粒子有贴图时，渲染该粒子
				...
				aParticle = mParticleSystem->mParticleHolder->mParticles.DataArrayTryToGet((uint32_t)theParticle->mCrossFadeParticleID);
...
{
	for (TodListNode<ParticleEmitterID>* aNode = mEmitterList.mHead; aNode != nullptr; aNode = aNode->mNext)
		mParticleHolder->mEmitters.DataArrayGet((uint32_t)aNode->mValue)->Draw(g);
}
...
void TodParticleEmitter::Draw(Graphics* g)
{
	...
	for (TodListNode<ParticleID>* aNode = mParticleList.mHead; aNode != nullptr; aNode = aNode->mNext)
		DrawParticle(g, mParticleSystem->mParticleHolder->mParticles.DataArrayGet((uint32_t)aNode->mValue), &aTriangleGroup);
...
{
	for (TodListNode<ParticleEmitterID>* aNode = mEmitterList.mHead; aNode != nullptr; aNode = aNode->mNext)
		mParticleHolder->mEmitters.DataArrayGet((uint32_t)aNode->mValue)->SystemMove(theX, theY);
}
...
void TodParticleEmitter::CrossFadeEmitter(TodParticleEmitter* theToEmitter)
{
	...
	for (TodListNode<ParticleID>* aNode = mParticleList.mHead; aNode != nullptr; aNode = aNode->mNext)
		CrossFadeParticle(mParticleSystem->mParticleHolder->mParticles.DataArrayGet((uint32_t)aNode->mValue), theToEmitter);
}
