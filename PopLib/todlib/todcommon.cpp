...
void TodSandImageIfNeeded(Image* theImage)
{
	...
	if (TestBit(aImage->mImageFlags, D3DIMAGEFLAG_SANDING))
	{
		...
		//((MemoryImage*)theImage)->mImageFlags &= ~D3DIMAGEFLAG_SANDING;
		SetBit((uint32_t&)aImage->mImageFlags, D3DIMAGEFLAG_SANDING, false);  // 清除标记
	}
}
...
ulong AverageNearByPixels(MemoryImage* theImage, ulong* thePixel, int x, int y)
{
	...
	for (int i = -1; i <= 1; i++)  // 依次循环上方、当前、下方的一行
	{
		...
		for (int j = -1; j <= 1; j++)  // 依次循环左方、当前、右方的一列
		{
			...
			{
				ulong aPixel = *(thePixel + i * theImage->mWidth + j);
...
void FixPixelsOnAlphaEdgeForBlending(Image* theImage)
{
	...
	if (aDuration > 20)
	{
		TodTraceAndLog("LOADING:Long sanding '%s' %d ms on %s", theImage->mFilePath.c_str(), aDuration, std::string(gGetCurrentLevelName()).c_str());
	}
}
...
bool TodResourceManager::TodLoadResources(const std::string& theGroup)
{
	...
	if (aDuration > 20)
	{
		TodTraceAndLog("LOADED: '%s' %d ms on %s", theGroup.c_str(), aDuration, std::string(gGetCurrentLevelName()).c_str());
	}
...
PopString TodReplaceNumberString(const PopString& theText, const PopChar* theStringToFind, int theNumber)
{
	...
	if (aPos != PopString::npos)
	{
		PopString aNumberString = StrFormat(_S("%d"), theNumber);
...
{
    if (!theBuffer || theSize <= 0 || !theFormat)
        return 0;

    int count = vsnprintf(theBuffer, theSize, theFormat, theArgList);
    if (count < 0 || count >= theSize)
    {
