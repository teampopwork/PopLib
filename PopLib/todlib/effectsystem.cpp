...
static uint32_t		gTodVertexReservoirUsed = 0;
...
//0x445D00
static inline uint32_t Tod_leClip(TriVertex** src, TriVertex** dst, const float edge)
{
	TriVertex** _dst = dst;
...
//0x445E00
static inline uint32_t Tod_reClip(TriVertex** src, TriVertex** dst, const float edge)
{
	TriVertex** _dst = dst;
...
//0x445F00
static inline uint32_t Tod_teClip(TriVertex** src, TriVertex** dst, const float edge)
{
	TriVertex** _dst = dst;
...
//0x446000
static inline uint32_t Tod_beClip(TriVertex** src, TriVertex** dst, const float edge)
{
	TriVertex** _dst = dst;
...
void TodTriangleGroup::DrawGroup(Graphics* g)
{
	if (mImage && mTriangleCount)
	{
		...
		if (GPUImage::Check3D(g->mDestImage))
		{
			...
			mImage->mDrawn = true;
			anImage->mRenderer->DrawTrianglesTex(mVertArray, mTriangleCount, Color::White, mDrawMode, mImage, 0.0f, 0.0f, g->mLinearBlend);
		}
		else
		{
...
void TodTriangleGroup::AddTriangle(Graphics* g, Image* theImage, const Matrix3& theMatrix, const Rect& theClipRect, const Color& theColor, int theDrawMode, const Rect& theSrcRect)
{
	...
	uint32_t aTexColor = theColor.ToInt();
