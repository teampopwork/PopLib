...
TodStringListFormat::TodStringListFormat(const char* theFormatName, Font** theFont, const Color& theColor, int theLineSpacingOffset, uint32_t theFormatFlags) : 
...
bool TodStringListReadName(const char*& thePtr, std::string& theName)
{
	...
	else
	{
		...
		theName = PopLib::Trim(string(aNameStart + 1, aCount));  // 取得中括号之间的部分并去除字符串前后的空白字符
		if (theName.size() == 0)
		{
...
bool TodStringListReadValue(const char*& thePtr, std::string& theValue)
{
	...
	theValue = PopLib::Trim(string(thePtr, aLen));  // 如果存在下一个“[”，则取到“[”前为止；否则，取剩下的全部
...
{
	std::string aNameString = theName;
	StringStringMap::iterator anItr = gAppBase->mStringProperties.find(aNameString);
	...
	{
		return PopLib::StrFormat(_S("<Missing %s>"), theName.c_str());
	}
}
