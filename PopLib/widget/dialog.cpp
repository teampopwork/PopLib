...
PopString PopLib::DIALOG_YES_STRING = "YES";
PopString PopLib::DIALOG_NO_STRING = "NO";
PopString PopLib::DIALOG_OK_STRING = "OK";
PopString PopLib::DIALOG_CANCEL_STRING = "CANCEL";
			   ...
			   const PopString &theDialogFooter, int theButtonMode)
{
	...
	if ((mButtonMode == BUTTONS_YES_NO) || (mButtonMode == BUTTONS_OK_CANCEL))
	{
		...
		{
			mYesButton->mLabel = gAppBase->GetString(("ID_YES"), DIALOG_YES_STRING);
			mNoButton->mLabel = gAppBase->GetString(("ID_NO"), DIALOG_NO_STRING);
		}
		else
		{
			mYesButton->mLabel = gAppBase->GetString(("ID_OK"), DIALOG_OK_STRING);
			mNoButton->mLabel = gAppBase->GetString(("ID_CANCEL"), DIALOG_CANCEL_STRING);
		}
	}
...
{
	if (mHeaderFont == NULL)
		mHeaderFont = new SysFont(gAppBase, LiberationSans_Regular, LiberationSans_Regular_Size, 14);
	if (mLinesFont == NULL)
		mLinesFont = new SysFont(gAppBase, LiberationSans_Regular, LiberationSans_Regular_Size, 12);
}
...
void Dialog::Resize(int theX, int theY, int theWidth, int theHeight)
{
	...
	if ((mYesButton != NULL) && (mNoButton != NULL))
	{
		...
		mYesButton->Resize(mX + mBackgroundInsets.mLeft + mContentInsets.mLeft + mButtonSidePadding,
						   mY + mHeight - mContentInsets.mBottom - mBackgroundInsets.mBottom - aBtnHeight, aBtnWidth,
						   aBtnHeight);
...
{
	// gAppBase->DoMainLoop(mId);

	while ((gAppBase->UpdateAppStep(NULL)) && (mWidgetManager != NULL) && (mResult == 0x7FFFFFFF))
		;

	if (autoKill)
		gAppBase->KillDialog(mId);
