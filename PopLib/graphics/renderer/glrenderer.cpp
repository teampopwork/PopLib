...
void GLRenderer::UpdateViewport()
{
	...
	mProjection = glm::ortho(0.0f, (float)mPresentationRect.mWidth, (float)mPresentationRect.mHeight, 0.0f, -1.0f, 1.0f) * glm::mat4(1.0f);
}
...
glm::vec2 RotatePointAroundPivot(const glm::vec2 point, const glm::vec2 center, float angleRad)
{
	...
	glm::vec2 rotation = 
	{
		translation.x * cosValue - translation.y * sinValue,
...
glm::vec2 TransformToPoint(float x, float y, const Matrix3 &m, float aTransX = 0, float aTransY = 0)
{
	glm::vec2 result;
							...
							int theDrawMode)
{
	...
	aCmd.mBlendMode = ChooseBlendMode(theDrawMode);
	aCmd.mClipRect = new Rect(0, 0, mPresentationRect.mWidth, mPresentationRect.mHeight);
	glm::vec4 color = glm::vec4(theColor.mRed / 255.0f, theColor.mGreen / 255.0f, theColor.mBlue / 255.0f, theColor.mAlpha / 255.0f);
...
void GLRenderer::FillPoly(const Point theVertices[], int theNumVertices, const Rect *theClipRect,
...
void GLRenderer::BltTexture(Texture *theTexture, const Rect &theSrcRect, const Rect &theDestRect,
							  const Color &theColor, int theDrawMode)
{
